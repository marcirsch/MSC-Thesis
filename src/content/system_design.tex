...
Just like in the case of 2D SLAM, in the first layout sensors are placed evenly to cover the whole sphere,
...
The project folder structure can be seen in figure \ref{fig:vscode_folder_structure}. Measurements folder
...
Since the VL53L1X sensors in a real-world application would be measuring simultaneously, the simulated
...
Ray instead which would require modifications in the filtering algorithm.
...
scans more efficiently. For example, SLAM is not expected to work properly on a corridor without any details
in range, because it needs a reference point that can be used for scan matching. If each scan looks
...
The second floor of the designed building is seen in figure \ref{fig:building_floor_map}. The ground
level has the same floor map except it also has a staircase in the top left room, windows,
and what is important, it has a ceiling. The ceiling is expected to play an important role in 3D slam,
...
The steps described above are for filtering a single sensor, but the same steps apply for multiple
...
$\hat{c_i}$ is one coordinate of the estimated pose and $c_i$ is the according coordinate
of the pose from the ground-truth trajectory.
