...
\begin{figure}[!h]
    ...
    \caption{Map and trajectory using 6 pieces of 4x4 resolution sensor with 400ms sampling time 
...
The best produced map using 4x4 resolution sensors is on figure \ref{fig:08_failed_map}. 
Similar maps were created using 3x3 resolution.

\begin{figure}[!h]
    \centering
	$\vcenter{\hbox{\includegraphics[height=50mm, keepaspectratio]{figures/08_map_8sensor_3x3.png}}}$
    $\vcenter{\hbox{\includegraphics[height=50mm, keepaspectratio]{figures/08_map_8sensor_4x4.png}}}$
...
using 3x3 resolution. The extracted trajectories can be inspected on figure \ref{fig:08_8p_trajectories}.
In different quantities, but both trajectories have an offset that cumulated after exiting the first
room, due to the shortening of corridors. This offset persists throughout the trajectories afterwards,
but the submaps are properly aligned relative to each other.

\begin{figure}[!h]
    \centering
	$\vcenter{\hbox{\includegraphics[height=65mm, keepaspectratio]{figures/08_trajectory_8p_3x3.png}}}$
...
algorithm needs to compensate these rotations. To allow rotations I have lowered the rotation\_weight 
parameter and found 15 to be an optimal value.


\begin{figure}[!h]
    \centering
	$\vcenter{\hbox{\includegraphics[height=65mm, keepaspectratio]{figures/09_map.png}}}$
    $\vcenter{\hbox{\includegraphics[height=65mm, keepaspectratio]{figures/09_pointcloud.png}}}$
    \caption{3D slam map and inserted points using unfiltered data}
    \label{fig:09_3d_map}
\end{figure}


The estimated trajectory against ground-truth is visible on figure \ref{fig:09_3d_trajectory}. The error of 
the estimation is relatively small and it typically stays under 15cm, but never goes above 30cm. 
The cumulated RMS error for the unfiltered setup is 10.4cm, with 3.6cm error on x, 4.4cm on y and 
2.4cm on z axis.
...
on figure \ref{fig:09_3d_pointclouds}. 
...
The artifact of transition points is better visible on 3D visualization on figure 
\ref{fig:10_transition_points}. These floating points are caused by the wide field of view and makes
...
\subsection{Effects of sampling rate}
To see the effects of lowered sampling rate I have kept the layout from previous simulations and 
applied sampling rate filter with a sampling time of 400ms. The produced map is on figure \ref{fig:11_map}.
A tilt can be observed on the bottom left of the map, but otherwise the map is consistent and was able to
...
The trajectory using 4x4 resolution sensors and sampling rate filter can be seen on figure \ref{fig:11_trajectory}.
As seen in previous measurements, the corridor is shortened when the drone exits the first room. 
In addition to this offset, the second corridor before the big room is also shortened that results 
...
The mapping quality hasn't improved significantly. The best quality maps using these setups are seen on 
...
resolution and 163.71cm for 3x3. The trajectories are on figure \ref{fig:12_trajectories}.
...
therefore the LIDARs fail to pick up signal from both sides of the room in the same time. Due to the lack
of reference a strong slip appears in the produced map, about 3m.
